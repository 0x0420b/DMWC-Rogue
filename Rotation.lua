...
local Player, Pet, Buff, Debuff, Spell, Target, Talent, Item, GCD, CDs, HUD, Enemy40Y, Enemy40YC, Enemy20Y, Enemy20YC, ComboPoints, HP, Enemy8YC, Enemy8Y, Enemy60Y, Enemy60YC
...
local function DEF()
	...
	------------------
	-- Evasion
	if Spell.Evasion:IsReady() and (DMW.Player.HP < Setting("Evasion Percent") and DMW.Player.Target and DMW.Player.Target.TTD > 5 or #DMW.Player:GetEnemies(8) > 1) then
...
function Rogue.Rotation()
	...
	if Setting("Kick") then
		for _, Unit in pairs(DMW.Attackable) do
			if Unit.Distance <= 5 and Unit:Interrupt() then
	...
	if Setting("AutoStealth") and Spell.Stealth:IsReady() and not Player.Combat and not Buff.Stealth:Exist(Player) and Enemy20YC > 0 then
	...
	if Spell.BloodFury:IsReady() and Player.Combat and Target and Target.TTD > 2 then
